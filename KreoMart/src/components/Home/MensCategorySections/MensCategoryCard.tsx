...
const MensCategoryCard: React.FC<BestCategoryCardProps> = ({ data }) => {
  ...
  const mensCategories: CategResult[] = [];
  if (data.navigation_header.some((header) => header.name === "Mens")) {
    mensCategories.push(data);
  }

  console.log(mensCategories, " Mens BestCategory  data is being displayed!!");

  return (
    <>
      {mensCategories.map((data: any) => (
        <Link
          key={data.id}
          href={{
